/** luna-spec-codegen: owned-layout */
export function FigmaSection_n_2270_14191() {
  return (
    <section data-figma-node="2270:14191" className="absolute box-border left-[337px] top-[2235px] mt-[306px] w-[1267px] h-[519px] flex flex-row items-center gap-12">
      <div data-figma-node="2264:10897" className="box-border w-[517px] h-[260px] relative gap-5">
        <div data-figma-node="2270:14185" className="box-border w-[517px] h-[260px] absolute left-[0px] top-[0px] gap-5">
          <div data-figma-node="2270:14187" className="box-border w-[517px] h-[260px] absolute left-[0px] top-[0px] gap-5">
            <a data-figma-node="2270:15213" href="#contact" className="box-border w-[128px] h-[54px] absolute left-[0px] top-[0px] rounded-full inline-flex items-center justify-center whitespace-nowrap pt-[8px] pr-[20px] pb-[8px] pl-[20px] hover:opacity-90" style={{backgroundImage: "linear-gradient(150.1deg, #0b0b0b 0.0%, rgba(11, 11, 11, 0) 100.0%)"}}><span className="font-almarai text-[28px] font-[400] leading-[31px] text-left whitespace-nowrap text-[#c8a47e] whitespace-nowrap">Step 01</span></a>
            <div data-figma-node="2270:14190" className="box-border w-[517px] h-[110px] absolute left-[0px] top-[74px]">
              <p data-figma-node="2264:10454" className="box-border w-[517px] h-[55px] absolute left-[0px] top-[0px] font-eb-garamond text-[42px] font-[500] leading-[55px] text-left whitespace-nowrap text-[#ffffff]">Browse the continuously</p>
              <p data-figma-node="2270:14188" className="box-border w-[517px] h-[55px] absolute left-[0px] top-[55px] font-eb-garamond text-[42px] font-[500] leading-[55px] text-left whitespace-nowrap text-[#c8a47e]">updated collection.</p>
            </div>
            <p data-figma-node="2264:10455" className="box-border w-[447px] h-[56px] absolute left-[0px] top-[204px] opacity-[0.6] font-almarai text-[18px] font-[400] leading-[28px] text-left text-[#ffffff]">Explore hundreds of hand-designed templates for social, email, and more. Save the ones that fit your style.</p>
          </div>
        </div>
      </div>
      <img data-figma-node="3795:12052" src="/assets/figma/3795-12052.png" alt="Content Library" className="box-border w-[700px] h-[519px] overflow-hidden rounded-[10px] max-w-none object-cover object-top bg-[#0e0d0d]" />
    </section>
  );
}
