...
>(({ className, ...props }, ref) => (
  <thead ref={ref} className={cn('sticky top-0 bg-card [&_tr]:border-b [&_tr]:border-border/20', className)} {...props} />
));
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn('border-b border-border/15 transition-colors hover:bg-white/5', className)}
      {...props}
    />
  ),
);
